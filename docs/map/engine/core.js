// Copied from temporal-film (core.js), with patches for the live game (docs/map). Engine code:
// MIT, Copyright (c) 2026 Alexey Fateev, see ../licenses/engine-MIT.txt.
// PATCHES (everything else is byte-identical to temporal-film/core.js):
//   1. Deleted the query-string sizing (core.js:31: `?ar=` / `?w=` called setFormat). A crafted link
//      could allocate a huge canvas; the game never sizes a canvas from the URL (GAME_SPEC §2, §9).
//   2. Deleted the TIMELINE & RUNTIME section (core.js:514-583): defineFilm, the render hooks
//      (__drawFrame/__frame/__grid/__strip/__wav/__ready), gridSheet, locate/frameKey, show/loop,
//      buildPlayer (it parsed an HTML string), renderWav and the v1 audio helpers note/noiseBurst/pentHz.
//   3. Added resetFrame(ctx): the frame-reset sequence of the film's drawFrame (core.js:559) as its
//      own function, for game/runtime.js.
'use strict';
// ============================================================
...
// ===================== FRAME RESET (game patch 3) =====================
// resetFrame: the context reset the film's drawFrame did before every frame (from core.js:559):
// identity transform, clear, logical transform (resetT), alpha 1, source-over, no dash, no filter,
// no shadow. The caller brackets the frame with ctx.save()/ctx.restore() as drawFrame did.
function resetFrame(ctx) {
  ctx.setTransform(1, 0, 0, 1, 0, 0); ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height); resetT(ctx);
  ctx.globalAlpha = 1; ctx.globalCompositeOperation = 'source-over'; ctx.setLineDash([]); ctx.filter = 'none'; ctx.shadowBlur = 0;
}
